...
type
  TfFrameBill_brick = class(TfFrameNormal)
...
//------------------------------------------------------------------------------
class function TfFrameBill_brick.FrameID: integer;
begin
  Result := cFI_FrameBill_brick;
end;

procedure TfFrameBill_brick.OnCreateFrame;
begin
  inherited;
  FUseDate := True;
...
procedure TfFrameBill_brick.OnDestroyFrame;
begin
...
//Desc: 数据查询SQL
function TfFrameBill_brick.InitFormDataSQL(const nWhere: string): string;
var nStr: string;
begin
  FEnableBackDB := True;
  ...
  begin
    Result := Result + 'Where (L_Date>=''$ST'' and L_Date <''$End'') and exists(select 1 from $dict d where d.d_name=''$BrickItem'' and d.d_desc=b.L_stockno)';
    nStr := ' And ';
  end else nStr := ' Where ';
  ...
  Result := MacroValue(Result, [MI('$dict', sTable_SysDict),MI('$BrickItem', sFlag_BrickItem)]);
end;

procedure TfFrameBill_brick.AfterInitFormData;
begin
  FUseDate := True;
end;

function TfFrameBill_brick.FilterColumnField: string;
begin
...
//Desc: 执行查询
procedure TfFrameBill_brick.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 未开始提货的提货单
procedure TfFrameBill_brick.N4Click(Sender: TObject);
begin
...
//Desc: 日期筛选
procedure TfFrameBill_brick.EditDatePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 查询删除
procedure TfFrameBill_brick.CheckDeleteClick(Sender: TObject);
begin
  InitFormData('');
end;
...
//------------------------------------------------------------------------------
//Desc: 开提货单
procedure TfFrameBill_brick.BtnAddClick(Sender: TObject);
var nP: TFormCommandParam;
begin
  CreateBaseFormItem(cFI_FormBill_brick, PopedomItem, @nP);
...
//Desc: 删除
procedure TfFrameBill_brick.BtnDelClick(Sender: TObject);
var nStr: string;
begin
...
//Desc: 打印提货单
procedure TfFrameBill_brick.N1Click(Sender: TObject);
var
...
procedure TfFrameBill_brick.PMenu1Popup(Sender: TObject);
begin
...
//Desc: 修改未进厂车牌号
procedure TfFrameBill_brick.N5Click(Sender: TObject);
...
//Desc: 修改封签号
procedure TfFrameBill_brick.N7Click(Sender: TObject);
...
//Desc: 调拨提货单
procedure TfFrameBill_brick.N3Click(Sender: TObject);
...
procedure TfFrameBill_brick.N10Click(Sender: TObject);
var nStr: string;
begin
  if cxView1.DataController.GetSelectedCount > 0 then
  begin
...
procedure TfFrameBill_brick.N11Click(Sender: TObject);
var nStr: string;
begin
  if cxView1.DataController.GetSelectedCount > 0 then
  begin
...
procedure TfFrameBill_brick.N13Click(Sender: TObject);
var nStr:string;
begin
  if cxView1.DataController.GetSelectedCount < 1 then
  begin
...
initialization
  gControlManager.RegCtrl(TfFrameBill_brick, TfFrameBill_brick.FrameID);
end.
