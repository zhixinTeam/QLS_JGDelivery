...
type
  TfFrameHYRecord_Slag = class(TfFrameNormal)
...
uses
  ...
  UFormDateFilter, UFormHYRecord_Slag;

class function TfFrameHYRecord_Slag.FrameID: integer;
begin
  Result := cFI_FrameStockRecord_Slag;
end;

procedure TfFrameHYRecord_Slag.OnCreateFrame;
begin
  inherited;
  InitDateRange(Name, FStart, FEnd);
end;

procedure TfFrameHYRecord_Slag.OnDestroyFrame;
begin
  SaveDateRange(Name, FStart, FEnd);
  inherited;
end;

function TfFrameHYRecord_Slag.InitFormDataSQL(const nWhere: string): string;
begin
  ...
  Result := MacroValue(Result, [MI('$SR', sTable_StockRecord_Slag),
...
//------------------------------------------------------------------------------
//Desc: 添加
procedure TfFrameHYRecord_Slag.BtnAddClick(Sender: TObject);
begin
  if ShowStockRecordAddForm then InitFormData('');
end;

//Desc: 编辑
procedure TfFrameHYRecord_Slag.BtnEditClick(Sender: TObject);
var nStr: string;
begin
...
//Desc: 删除
procedure TfFrameHYRecord_Slag.BtnDelClick(Sender: TObject);
...
begin
  ...
  nSQL := Format(nSQL, [sTable_StockRecord_Slag, nStr]);
...
//Desc: 日期筛选
procedure TfFrameHYRecord_Slag.EditDatePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 查询
procedure TfFrameHYRecord_Slag.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 查看
procedure TfFrameHYRecord_Slag.cxView1DblClick(Sender: TObject);
var nStr: string;
begin
...
initialization
  gControlManager.RegCtrl(TfFrameHYRecord_Slag, TfFrameHYRecord_Slag.FrameID);
end.
