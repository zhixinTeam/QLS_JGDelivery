...
function TTruckQueueDBReader.IsStockMatch(nTruck: PTruckItem;
  nLine: PLineItem): Boolean;
begin
  Result := (nTruck.FStockNo = nLine.FStockNo) and (nTruck.FTICenterID=nLine.FLICenterID);
  if not Result then
  begin
    ...
    Result := Result and (nTruck.FTICenterID=nLine.FLICenterID);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FLICenterID   := FieldByName('Z_CenterID').AsString;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if FTruckPool[i].FTICenterID <> FLICenterID then Continue;
        //2.生产线不匹配
...
procedure TTruckQueueDBReader.FillCenterIDs;
var
  nIdx:Integer;
  function GetCenterID(const nBill: string):string;
  var
    nStr:string;
  begin
...
begin
  for nIdx := Low(FTruckPool) to High(FTruckPool) do
  begin
    FTruckPool[nIdx].FTICenterID := GetCenterID(FTruckPool[nIdx].FBill);
  end;
end;
