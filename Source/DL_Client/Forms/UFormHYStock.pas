...
procedure TfFormHYStock.initCbbStandard;
var
  nStr :string;
begin
  nStr := 'select * from %s where d_name=''%s'' and d_memo=''%s''';
  nStr := Format(nStr,[sTable_SysDict,sFlag_SysParam,sFlag_Standard]);
  with fdm.QuerySQL(nStr) do
  begin
    while not eof do
    begin
      cbbStandard.Properties.Items.Add(FieldByName('D_value').AsString);
      next;
    end;
  end;
end;
