...
uses
  ...
  UFormInputbox,uformChangeZtline;
...
begin
  Result := ' Select zt.*,Z_Name,L_CusID,L_CusName,L_Status,L_Value,b.l_invcenterid ' +
            'From $ZT zt ' +
...
var nStr,nLine,nTmp: string;
  nQHD:Boolean; //宏达建材
  nStockno,nOldLine,ncenterid:string;
  nObj:TFormChangeZtLine;
begin
  nQHD := False;
  {$IFDEF QHSN}
  {$IFDEF GGJC}
  ...
  nOldLine := SQLQuery.FieldByName('T_Line').AsString;
  ncenterid := SQLQuery.FieldByName('l_invcenterid').AsString;
  nLine := nStr;

  if nQHD then
  begin
    nObj := TFormChangeZtLine.Create(nil);
    try
      nObj.OldLine := nOldLine;
      nObj.StockNo := nStockno;
      nObj.CenterId := ncenterid;
      if nObj.ShowModal=mrCancel then Exit;
      nLine := nObj.NewLine;
    finally
      nObj.Free;
    end;
