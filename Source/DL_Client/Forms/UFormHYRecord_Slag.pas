...
type
  TfFormHYRecord_Slag = class(TForm)
...
function ShowStockRecordAddForm: Boolean;
begin
  with TfFormHYRecord_Slag.Create(Application) do
  begin
    FRecordID := '';
    Caption := '矿渣粉检验记录 - 添加';
...
begin
  ...
  begin
    gForm := TfFormHYRecord_Slag.Create(Application);
...
//------------------------------------------------------------------------------
procedure TfFormHYRecord_Slag.FormCreate(Sender: TObject);
var nIni: TIniFile;
begin
  ResetHintAllForm(Self, 'E', sTable_StockRecord_Slag);
  //重置表名称
...
procedure TfFormHYRecord_Slag.FormClose(Sender: TObject;
...
procedure TfFormHYRecord_Slag.BtnExitClick(Sender: TObject);
begin
  Close;
end;

procedure TfFormHYRecord_Slag.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfFormHYRecord_Slag.cxTextEdit17KeyPress(Sender: TObject;
  var Key: Char);
begin
...
//------------------------------------------------------------------------------
procedure TfFormHYRecord_Slag.GetData(Sender: TObject; var nData: string);
begin
  if Sender = EditDate then nData := DateTime2Str(EditDate.Date);
end;

function TfFormHYRecord_Slag.SetData(Sender: TObject; const nData: string): Boolean;
begin
...
//Parm: 记录编号
//Desc: 载入nID供应商的信息到界面
procedure TfFormHYRecord_Slag.InitFormData(const nID: string);
var nStr: string;
begin
  EditDate.Date := Now;
...
//  cbxHhcl.Text:= '请选择';
  ...
  begin
    ...
    nStr := Format(nStr, [sTable_StockRecord_Slag, nID]);
    LoadDataToForm(FDM.QuerySQL(nStr), Self, '', SetData);
  end;
end;

//Desc: 设置类型
procedure TfFormHYRecord_Slag.EditStockPropertiesEditValueChanged(Sender: TObject);
var nStr: string;
begin
  if FRecordID = '' then
  begin
...
//Desc: 生成随机编号
procedure TfFormHYRecord_Slag.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 保存数据
procedure TfFormHYRecord_Slag.BtnOKClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      EditID.SetFocus;
      ShowMsg('该编号的记录已经存在于通用熟料检验记录中', sHint);
      Exit;
    end;

    nSQL := MakeSQLByForm(Self, sTable_StockRecord_Slag, '', True, GetData);
  end else
  begin
    EditID.Text := FRecordID;
    nStr := 'R_ID=''' + FRecordID + '''';
    nSQL := MakeSQLByForm(Self, sTable_StockRecord_Slag, nStr, False, GetData);
  end;
