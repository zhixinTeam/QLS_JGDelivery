...
begin
  ...
  Result := 'Select *,(D_MValue-D_PValue-Isnull(D_KZValue,0)) as D_NetWeight' +
            ' From $OD od Inner Join $OO oo on od.D_OID=oo.O_ID left outer join $OB ob on oo.o_project=ob.b_id ';
  //xxxxxx
  ...
  Result := MacroValue(Result, [MI('$OD', sTable_OrderDtl),MI('$OO', sTable_Order),
            MI('$S', Date2Str(FStart)), MI('$End', Date2Str(FEnd + 1)),MI('$OB', sTable_OrderBase)]);
  //xxxxx
end;
