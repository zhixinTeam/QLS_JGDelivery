...
type
  ...
  public
    ...
    procedure SplitCameraTunnel(const nTunnel: PReaderHost; const nStr: string);
...
begin
  ...
  try
    ...
    nTmp := nXML.Root.FindNode('cameras');
    if Assigned(nTmp) then
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          nTP := nNode.FindNode('camera');
          if Assigned(nTP) then
          begin
            nStr := nTP.AttributeByName['id'];
            FCamera := GetCamera(nStr);
            SplitCameraTunnel(nHost, nTP.ValueAsString);
          end else
          begin
            FCamera := nil;
          end;
...
function T02NReader.GetCamera(const nID: string): PPTCameraItem;
var nIdx: Integer;
begin
  Result := nil;
...
procedure T02NReader.SplitCameraTunnel(const nTunnel: PReaderHost;
  const nStr: string);
var nIdx: Integer;
    nList: TStrings;
begin
