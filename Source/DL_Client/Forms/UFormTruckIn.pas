...
begin
  ...
  begin
    ...
    else if gCardUsed = sFlag_Other then
    begin
      nRet := GetPurchaseOrders(nStr, sFlag_TruckIn, gBills);
      for nIdx:=Low(gBills) to High(gBills) do
      begin
        gBills[nIdx].FSelected := True;
      end;
    end;
  ...
  nHint := '';
  for nIdx:=Low(gBills) to High(gBills) do
  if (gBills[nIdx].FStatus <> sFlag_TruckNone) and (gBills[nIdx].FNextStatus <> sFlag_TruckIn) then
  begin
    nStr := '※.单号:[ %s ] 状态:[ %-6s -> %-6s ]   ';
...
var nRet: Boolean;
    nFoutData:string;
begin
  if (gCardUsed = sFlag_Provide) or (gCardUsed = sFlag_Other) then
