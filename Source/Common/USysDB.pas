...
ResourceString     
  ...
  sFlag_wuchaType_D   = '1';                         //称重误差类型-包装水泥
  sFlag_wuchaType_S   = '2';                         //称重误差类型-散装水泥
  sFlag_wuchaType_Z   = '3';                         //称重误差类型-砖块
  ...
  sFlag_PDaiWuChaStop = 'PoundDaiWuChaStop';         //袋装误差时停止业务
  sFlag_PSanWuChaStop = 'PoundSanWuChaStop';         //散装误差时停止业务
  sFlag_PBrickWuChaStop = 'PoundBrickWuChaStop';     //砖块误差时停止业务
  ...
  sFlag_brickcolor    = 'BrickColor';                //外销砖块颜色
  ...
  sFlag_Standard      = 'standard';                  //化验结果执行标准
...
//  sFlag_DepBangFang   = '磅房';                      //磅房  
  ...
  sTable_StockRecord_Slag  = 'S_StockRecord_Slag';   //矿渣粉检验记录
  sTable_StockRecord_Concrete  = 'S_StockRecord_Concrete';   //混凝土制品检验记录
  sTable_StockRecord_clinker  = 'S_StockRecord_clinker';     //通用熟料检验记录
  ...
  *.wcb_WebMallStatus:是否开通商城用户，默认值0：未开通 1：已开通
   ...
   *.R_ID:记录编号
	 ...
	 *.R_Density:密度
	 *.R_BiBiao:比表面积
	 *.R_ActivityIndex_7d:活性指数7D
	 *.R_ActivityIndex_28d:活性指数28D
	 *.R_FluidityRatio:流动度比
	 *.R_WaterContent:含水量
	 *.R_SO3:三氧化硫
	 *.R_CL:氯离子
	 *.R_ShaoShi:烧失量
  ...
  sSQL_NewStockRecord_Concrete = 'Create Table $Table(R_ID $Inc,'
  		...
  		+'R_BatValid char(1) not null default(''Y''))';
  {-----------------------------------------------------------------------------
   混凝土制品检验记录:StockRecord_Concrete
   *.R_ID:记录编号
	 *.R_SerialNo:试样编号
	 *.R_PID:产品编号
	 *.R_Stockname:产品名称
	 *.R_Date:录入日期
	 *.R_StrengthGrade:强度等级	
	 *.R_Specifications:规格型号
	 *.R_BatQuaStart:批量
	 *.R_AppearanceQuality:外观质量	
	 *.R_DimensionalDeviation:尺寸偏差	
	 *.R_WaterAbsorption:吸水率(%)	
	 *.R_AvgCompressiveStrength:抗压强度(Mpa)平均值
	 *.R_MinCompressiveStrength:抗压强度(Mpa)单块最小值
	 *.R_Conclusion:检测结论及意见
	 *.R_Remark:备注
	 *.R_Man:录入人
	 *.R_CenterID:生产线ID
	 *.R_BatQuaEnd:预警量
