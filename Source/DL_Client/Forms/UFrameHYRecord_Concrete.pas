...
type
  TfFrameHYRecord_Concrete = class(TfFrameNormal)
...
uses
  ...
  UFormDateFilter, UFormHYRecord_Concrete;

class function TfFrameHYRecord_Concrete.FrameID: integer;
begin
  Result := cFI_FrameStockRecord_Concrete;
end;

procedure TfFrameHYRecord_Concrete.OnCreateFrame;
begin
  inherited;
  InitDateRange(Name, FStart, FEnd);
end;

procedure TfFrameHYRecord_Concrete.OnDestroyFrame;
begin
  SaveDateRange(Name, FStart, FEnd);
  inherited;
end;

function TfFrameHYRecord_Concrete.InitFormDataSQL(const nWhere: string): string;
begin
  ...
  Result := MacroValue(Result, [MI('$SR', sTable_StockRecord_Concrete),
...
//------------------------------------------------------------------------------
//Desc: 添加
procedure TfFrameHYRecord_Concrete.BtnAddClick(Sender: TObject);
begin
  if ShowStockRecordAddForm then InitFormData('');
end;

//Desc: 编辑
procedure TfFrameHYRecord_Concrete.BtnEditClick(Sender: TObject);
var nStr: string;
begin
...
//Desc: 删除
procedure TfFrameHYRecord_Concrete.BtnDelClick(Sender: TObject);
...
begin
  ...
  nSQL := Format(nSQL, [sTable_StockRecord_Concrete, nStr]);
...
//Desc: 日期筛选
procedure TfFrameHYRecord_Concrete.EditDatePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 查询
procedure TfFrameHYRecord_Concrete.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 查看
procedure TfFrameHYRecord_Concrete.cxView1DblClick(Sender: TObject);
var nStr: string;
begin
...
initialization
  gControlManager.RegCtrl(TfFrameHYRecord_Concrete, TfFrameHYRecord_Concrete.FrameID);
end.
