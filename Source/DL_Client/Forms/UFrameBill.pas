...
begin
  ...
  begin
    Result := Result + 'Where (L_Date>=''$ST'' and L_Date <''$End'') and not exists(select 1 from $dict d where d.d_name=''$BrickItem'' and d.d_desc=b.L_stockno)';
    nStr := ' And ';
  end else nStr := ' Where ';
...
begin
  ...
  begin
    ...
    nCode := 'QHD'+nCode+Fields[2].AsString+Fields[3].AsString;
    ShowDlg(nCode, sHint);
  end;
end;
