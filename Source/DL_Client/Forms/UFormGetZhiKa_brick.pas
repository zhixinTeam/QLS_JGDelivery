...
type
  TfFormGetZhiKa_brick = class(TfFormNormal)
...
class function TfFormGetZhiKa_brick.CreateForm(const nPopedom: string;
...
begin
  ...
  with TfFormGetZhiKa_brick.Create(Application) do
  try
    Caption := '选择纸卡';
    InitFormData('');
...
class function TfFormGetZhiKa_brick.FormID: integer;
begin
  Result := cFI_FormGetZhika_brick;
end;

procedure TfFormGetZhiKa_brick.FormCreate(Sender: TObject);
begin
...
procedure TfFormGetZhiKa_brick.FormClose(Sender: TObject;
...
//------------------------------------------------------------------------------
procedure TfFormGetZhiKa_brick.InitFormData(const nID: string);
begin
...
//Desc: 清理客户信息
procedure TfFormGetZhiKa_brick.ClearCustomerInfo;
begin
...
//Desc: 载入nID客户的信息
function TfFormGetZhiKa_brick.LoadCustomerInfo(const nID: string): Boolean;
var nDS: TDataSet;
...
procedure TfFormGetZhiKa_brick.EditNamePropertiesEditValueChanged(Sender: TObject);
begin
...
procedure TfFormGetZhiKa_brick.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TfFormGetZhiKa_brick.EditZKPropertiesEditValueChanged(Sender: TObject);
var nStr: string;
    i,nIdx: Integer;
    nNewPrice: Double;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if Fields[3].AsString='S' then
          FStockName := Fields[0].AsString+'散装'
        else
          FStockName := Fields[0].AsString;
        if FShowPrice then
...
//Desc: 选择客户
procedure TfFormGetZhiKa_brick.EditNameKeyPress(Sender: TObject; var Key: Char);
var nStr: string;
...
procedure TfFormGetZhiKa_brick.BtnOKClick(Sender: TObject);
var
  nIdx:Integer;
begin
...
procedure TfFormGetZhiKa_brick.EditZKKeyPress(Sender: TObject; var Key: Char);
var nStr: string;
    nP: TFormCommandParam;
begin
  if Key = #13 then
...
initialization
  gControlManager.RegCtrl(TfFormGetZhiKa_brick, TfFormGetZhiKa_brick.FormID);
end.
