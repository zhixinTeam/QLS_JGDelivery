...
begin
  ...
  if FQueryHas then
  begin
    nStr := 'Select bc.*,L_ID,L_CusID,L_CusName,L_Truck,l_ifneidao From $BC bc ' +
...
begin
  ...
  nNeiDao := SQLQuery.FieldByName('L_ifneidao').AsString;
  nStr := Format('确定要对卡[ %s ]执行销卡操作吗?', [nCard]);
  if not QueryDlg(nStr, sAsk) then Exit;

  if LogoutBillCard(nCard,nNeiDao) then
  begin
    InitFormData(FWhere);
