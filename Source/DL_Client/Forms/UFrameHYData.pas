...
type
  TfFrameHYData = class(TfFrameNormal)
    ...
    procedure EditDatePropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
    procedure N371Click(Sender: TObject);
  private
...
begin
  ...
  begin
    nStr := SQLQuery.FieldByName('H_ID').AsString;
    nHydan := SQLQuery.FieldByName('H_Serialno').AsString;
    PrintHuaYanReport(nStr, False,nHydan);
  end;
end;
...
procedure TfFrameHYData.N371Click(Sender: TObject);
var nStr: string;
  nHydan:string;
begin
  if cxView1.DataController.GetSelectedCount > 0 then
  begin
    nStr := SQLQuery.FieldByName('H_ID').AsString;
    nHydan := SQLQuery.FieldByName('H_Serialno').AsString;
    PrintHeGeReport(nStr, False,nHydan);
  end;
end;
