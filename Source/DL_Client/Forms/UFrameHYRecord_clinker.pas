...
type
  TfFrameHYRecord_clinker = class(TfFrameNormal)
...
uses
  ...
  UFormDateFilter, UFormHYRecord_clinker;

class function TfFrameHYRecord_clinker.FrameID: integer;
begin
  Result := cFI_FrameStockRecord_clinker;
end;

procedure TfFrameHYRecord_clinker.OnCreateFrame;
begin
  inherited;
  InitDateRange(Name, FStart, FEnd);
end;

procedure TfFrameHYRecord_clinker.OnDestroyFrame;
begin
  SaveDateRange(Name, FStart, FEnd);
  inherited;
end;

function TfFrameHYRecord_clinker.InitFormDataSQL(const nWhere: string): string;
begin
  ...
  Result := MacroValue(Result, [MI('$SR', sTable_StockRecord_clinker),
...
//------------------------------------------------------------------------------
//Desc: 添加
procedure TfFrameHYRecord_clinker.BtnAddClick(Sender: TObject);
begin
  if ShowStockRecordAddForm then InitFormData('');
end;

//Desc: 编辑
procedure TfFrameHYRecord_clinker.BtnEditClick(Sender: TObject);
var nStr: string;
begin
...
//Desc: 删除
procedure TfFrameHYRecord_clinker.BtnDelClick(Sender: TObject);
...
begin
  ...
  nSQL := Format(nSQL, [sTable_StockRecord_clinker, nStr]);
...
//Desc: 日期筛选
procedure TfFrameHYRecord_clinker.EditDatePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 查询
procedure TfFrameHYRecord_clinker.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 查看
procedure TfFrameHYRecord_clinker.cxView1DblClick(Sender: TObject);
var nStr: string;
begin
...
initialization
  gControlManager.RegCtrl(TfFrameHYRecord_clinker, TfFrameHYRecord_clinker.FrameID);
end.
